...
date: "Initial Version: 4/16/2021 Current Version: `r format(Sys.Date(), '%m %d %Y')`"
output: 
  html_document:
    toc: true
    toc_depth: 6
...
This page is to showcase the work of the NBP2019 research voyage. Only important code and conclusions are to be presented here. All prior iterations and the process by which I get to the final version of the manuscript are present in the [Github Repository](https://github.com/ccarnivale/Antarctic_mixotrophy).
...
### Data Setup and Tidying

Initial Response Variables calculated:
...
```{r Complete dataset read in, results='hide'}
#Old Groups can replace with next line at any time
# 2 Grouping methods were attempted
# THe first being groups identified within the Grattenpanche et al. 2022 paper
# Second using groups identified within this dataset which is clarified later in the document

# Grattepanche (2022) grouping
...
# Investigating the data structure once read in
str(ant_community_final)

# Need to convert character columns into workable factors for analyses
# Fixed data structure issues. Converting character to factors for categorical variables.
ant_community_final$Rep <- as.factor(ant_community_final$Rep)
ant_community_final$station <- as.factor(ant_community_final$station)
ant_community_final$depth <- factor(ant_community_final$depth, levels = c("Surf", "DCM", "Bucket"))
...
```{r Initial calculations standardized to per mL}
# From initial data collection, additional features need to be calculated for analysis.
...
```{r PCA Analysis of Environmental Variables}
...
-microsphere per ml had no correlation with any measured response variables.
-Increases in mixo activity and abundance correlate with positively with HNAN activity but not abundance.
-Mixotrophic abundance and activity are the only responses not correlated with mixing depth
...
chlA_cruise <- read.csv("/Users/christophercarnivale/Desktop/Dissertation_data/Antarctica_community_data/LTER_PP_data/Chlorophyll_Cruise.csv")
C_fixed_cruise <- read.csv("/Users/christophercarnivale/Desktop/Dissertation_data/Antarctica_community_data/LTER_PP_data/Primary Production_Cruise.csv")
